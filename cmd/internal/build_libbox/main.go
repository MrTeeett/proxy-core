...
func init() {
	...
	sharedFlags = append(sharedFlags, "-ldflags", "-X proxy-core/constant.Version="+currentTag+" -s -w -buildid=")
	debugFlags = append(debugFlags, "-ldflags", "-X proxy-core/constant.Version="+currentTag)
...
func buildAndroid() {
	...
	copyPath := filepath.Join("..", "proxy-core-for-android", "app", "libs")
...
func buildApple() {
	...
	copyPath := filepath.Join("..", "proxy-core-for-apple")
	if rw.IsDir(copyPath) {
