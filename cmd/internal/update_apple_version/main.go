...
func main() {
	...
	if flagRunInCI {
		...
		applePath = "../proxy-core-for-apple"
	}
	...
	projectFile := common.Must1(os.Open("proxy-core.xcodeproj/project.pbxproj"))
	var project map[string]any
	decoder := plist.NewDecoder(projectFile)
	common.Must(decoder.Decode(&project))
	objectsMap := project["objects"].(map[string]any)
	projectContent := string(common.Must1(os.ReadFile("proxy-core.xcodeproj/project.pbxproj")))
	...
	if updated0 || updated1 || updated2 {
		common.Must(os.WriteFile("proxy-core.xcodeproj/project.pbxproj", []byte(newContent), 0o644))
	}
}
