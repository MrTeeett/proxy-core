...
import (
	...
	"proxy-core/experimental/deprecated"
	"proxy-core/log"
	"proxy-core/option"
...
func NewDefaultDNSRule(ctx context.Context, logger log.ContextLogger, options option.DefaultDNSRule) (*DefaultDNSRule, error) {
	...
	if len(options.Geosite) > 0 {
		return nil, E.New("geosite database is deprecated in proxy-core 1.8.0 and removed in proxy-core 1.12.0")
	}
	if len(options.SourceGeoIP) > 0 {
		return nil, E.New("geoip database is deprecated in proxy-core 1.8.0 and removed in proxy-core 1.12.0")
	}
